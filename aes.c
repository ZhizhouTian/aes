...
static int aes_encrypt_block(aes_context *ctx, uint8_t cipher_text[BLOCK_SIZE], const uint8_t text[BLOCK_SIZE])
{
	uint32_t Nr = 0;
	uint32_t *RK = NULL;
	uint8_t *state = NULL;
...
static int aes_decrypt_block(aes_context *ctx, uint8_t text[BLOCK_SIZE], const uint8_t cipher_text[BLOCK_SIZE])
{
	uint32_t Nr = 0;
...
int aes_encrypt_all(aes_context *ctx, char *output, char *input, int size)
{
	...
	if (!ctx || !output || !input || !size)
		return -EINVAL;

	if (output == input)
		return -EINVAL;

	for (i = 0; i < roundup(size, BLOCK_SIZE) / BLOCK_SIZE; i++) {
		int ret = aes_encrypt_block(ctx, output + i * BLOCK_SIZE, input + i * BLOCK_SIZE);
		if (ret)
			return ret;
	}
...
int aes_decrypt_all(aes_context *ctx, char *output, char *input, int size)
{
	...
	for (i = 0; i < roundup(size, BLOCK_SIZE) / BLOCK_SIZE; i++) {
		int ret = aes_decrypt_block(ctx, output + i * BLOCK_SIZE, input + i * BLOCK_SIZE);
		if (ret)
			return ret;
	}
